...
- `Data+ML/test/6.2-6.5_DB/fix_plan.md`
- `Data+ML/test/6.2-6.5_DB/[CN]fix_plan.md`
- `Data+ML/test/6.2-6.5_DB/fix_summary.md`

2026-06-03 的方案在“修改 Schema”与“仅修改 API 映射”之间选择了前者，
执行顺序为：

1. 扩展 `venues.venue_type`。
2. 增加场馆展示、语言、无障碍和预警字段。
3. 增加报告、确认和拥挤度字段。
4. 创建 `venue_accessibility`、`venue_language`、`venue_warnings`。
5. 同步 Schema 文件。
6. 再完成 mock data、API serialization 和测试。

该方案形成了当前 13 表数据库的早期基础，但不能直接作为 Final 实施计划。
后续需求评审后的裁决如下：

| 历史方案 | 当前裁决 |
|---|---|
| `anonymous`、`reported_by` 表示报告人 | 改为认证 `user_id` 外键；账户删除必须覆盖关联数据 |
| `report_confirmations.language` | 可保留，但不能替代 `(report_id, user_id)` 唯一约束 |
| `forecast_1h/4h/8h` 标量列 | 改为正规时序预测结构；历史 `forecast_4h/8h` 已删除 |
| 服务器端医疗 Profile | 不实施；医疗资料默认只保存在客户端 |
| API 字段全部落库 | `open_now`、颜色、计数、badge 等继续由 API 查询/计算 |
| 多份 Schema 文件直接同步 | 必须先确认目标环境；现有 SQL 副本并不完全一致 |

因此，新开发应以本文件第 3、5、7 节为实施依据；历史 fix plan 只用于解释
已有字段和表的来源。
