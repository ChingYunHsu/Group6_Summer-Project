...
def test_mta_normalize_raw_bytes():
    """When gtfs-realtime-bindings is not available, raw bytes records the size."""
    result = efi.normalize_mta_gtfs_rt(b"fake protobuf data")
    # Should handle gracefully — either decode or record raw
    assert "mta_gtfs_rt_available" in result
    assert not result["mta_arrival_missing"]
