...
def normalize_mta_gtfs_rt(raw_bytes: bytes | None) -> dict[str, Any]:
    ...
    For Sprint 4: decode the raw protobuf feed if the gtfs-realtime-bindings
    library is available; otherwise log the raw byte count and return fallback
    with mta_gtfs_rt_available=True so the data pipeline knows the feed responded.
    """
    ...
    try:
        ...
        # Library not installed — just record availability
        print("  [mta] gtfs-realtime-bindings not installed; recording raw fetch only")
        return {
            "mta_service_disruption_flag": 0,
            "mta_disruption_missing": True,
